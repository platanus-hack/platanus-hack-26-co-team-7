...
from sqlalchemy import CheckConstraint, DateTime, Enum, Float, ForeignKey, Index, Integer, String, UniqueConstraint, func
...
class GatewayTelegramRecord(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_gateway_sync_records_key_id", "key_id"),
...
class PersonState(Base):
    ...
    event_id: Mapped[str | None] = mapped_column(String, ForeignKey("events.event_id", ondelete="CASCADE"))
    user_id: Mapped[str] = mapped_column(String, nullable=False)
    ...
    __table_args__ = (UniqueConstraint("event_id", "user_id", name="uq_gateway_sync_person_states_event_user"),)
